...
public class MainActivity extends AppCompatActivity {
    ...
    static final int MIN_DISTANCE_FOR_X = 100;
    static final int MIN_DISTANCE_FOR_Y = 50;
            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                switch (action){
                    ...
                    case(MotionEvent.ACTION_UP):
                        x1 = event.getX();
                        y1 = event.getY();
                        float X_difference = x1 - x;
                        float Y_difference = y1 - y;
                        if(Math.abs(X_difference) > MIN_DISTANCE_FOR_X){
                            if((x1 > x) && (Math.abs(Y_difference) < MIN_DISTANCE_FOR_Y)){
                                //Toast.makeText(getApplicationContext(),"left2right" + Y_difference,Toast.LENGTH_SHORT).show();
                                addItem();
                                return true;
                            }else if((x1 < x) && (Math.abs(Y_difference) < MIN_DISTANCE_FOR_Y)){
                                //Toast.makeText(getApplicationContext(),"right2left" + Y_difference,Toast.LENGTH_SHORT).show();
                                clearAll();
                                return true;
                            }
                        }
